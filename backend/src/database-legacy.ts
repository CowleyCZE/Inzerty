...
const ensurePostgresColumns = async (pool: any) => {
  ...
  await pool.query('ALTER TABLE ads ADD COLUMN IF NOT EXISTS source TEXT DEFAULT \'bazos_cz\'');
  await pool.query('ALTER TABLE ads ADD COLUMN IF NOT EXISTS external_id TEXT');
  await pool.query('ALTER TABLE ads ADD COLUMN IF NOT EXISTS posted_at TIMESTAMPTZ');
  await pool.query('ALTER TABLE ads ADD COLUMN IF NOT EXISTS seller_info JSONB');
  await pool.query('ALTER TABLE ads ADD COLUMN IF NOT EXISTS metadata JSONB');
};
...
const rebuildSqliteAdsForCompositeUnique = async (db: Database) => {
  ...
  try {
    ...
    await db.exec(`
      INSERT OR IGNORE INTO ads_new (id, title, price, price_value, location, description, date_posted, url, image_url, ad_type, brand, scraped_at, model_ai, embedding, source)
      SELECT id, title, price, price_value, location, description, date_posted, url, image_url, ad_type, brand, scraped_at, model_ai, embedding, 'bazos_cz'
      FROM ads
